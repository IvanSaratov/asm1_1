...
.data
array db 20 dup (0) ;исходный массив
startNum equ 2
len equ 10
.code
...
start:
	...
	init: 						; Заполняем первые len-элементов значениями
		mov array[SI], AL 		; Заносим в массив элемент AL
		push AX
		mul AL					; Вычисляем квадрат
		mov array[SI+len], AL	; Так же сразу заносим его квадрат
		pop AX
		add AL, 2 				; увеличение значения следуещего элемента массива на 2, так как нам нужны четные числа
	...
	show1: 			
		...
		show2:
			mov AL, array[si] 		; Значение элемента массива помещается в AL
